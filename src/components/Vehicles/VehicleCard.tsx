...
const statusColors = {
  ...
  rented: { bg: 'bg-orange-50', text: 'text-orange-800', badge: 'bg-orange-100 text-orange-800' },
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">{vehicle.year} - {vehicle.color} - {vehicleTypeLabels[vehicle.vehicle_type]}</p>
          </div>
        </div>
      ...
      <div className="space-y-2">
        ...
        {vehicle.current_client && (
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span className="ml-1 truncate">{vehicle.current_client}</span>
          </div>
        )}
        ...
        {vehicle.next_maintenance_mileage && (
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span className="font-medium">Prox. mant.:</span>
        ...
        {vehicle.technical_review_expiry && (
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span className="font-medium">Rev. tecnica:</span>
            <span className={`ml-1 ${technicalReviewExpiring ? 'text-red-600 font-semibold' : ''}`}>
              {new Date(vehicle.technical_review_expiry).toLocaleDateString()}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs text-gray-500">
              {Math.round(((vehicle.mileage - vehicle.last_maintenance_mileage) / vehicle.maintenance_interval) * 100)}%
            </span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all duration-300 ${needsMaintenance ? 'bg-red-500' : 'bg-blue-500'}`}
              style={{ width: `${Math.min(100, ((vehicle.mileage - vehicle.last_maintenance_mileage) / vehicle.maintenance_interval) * 100)}%` }}
            ></div>
          </div>
        </div>
      )}
